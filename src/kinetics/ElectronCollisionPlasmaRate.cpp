...
ElectronCollisionPlasmaRate::ElectronCollisionPlasmaRate(const double* energyLevels,
                                                         const double* crossSection,
                                                         size_t length)
{
    m_energyLevels = Eigen::Map<const Eigen::ArrayXd>(energyLevels, length);
    m_crossSection = Eigen::Map<const Eigen::ArrayXd>(crossSection, length);
}
...
void ElectronCollisionPlasmaRate::setParameters(const AnyMap& node, const UnitStack& rate_units)
{
    ...
    if (node.hasKey("energy-levels")) {
        vector_fp levels = node["energy-levels"].asVector<double>();
    ...
    if (node.hasKey("cross-section")) {
        vector_fp crossSection = node["cross-section"].asVector<double>();
